...
void comprobar_vehiculo(hash_t* garage, const char* patente){
  printf("El vehiculo patente %s es: ", patente);
  void* retorno = hash_obtener(garage, patente);
  printf("%s\n", (char*)retorno);
}

int ejemplo_profesores(){
  ...
  printf("\n");
  printf("CANTIDAD DE AUTOS: %li",hash_cantidad(garage));
  printf("\n\n");
  ...
  printf("Retiro uno que no existe debe salir error \n");
  quitar_vehiculo(garage, "QDM443");
  printf("\n");

  printf("Busco los autos recién sacados\n");

  verificar_vehiculo(garage, "QDM443", false);
  verificar_vehiculo(garage, "PQO697", false);
  printf("\n");

  guardar_vehiculo(garage, "PQO697", "Auto de Juan otra vez");
  guardar_vehiculo(garage, "PQO697", "Auto de Juan otra vez, otra vez");
  verificar_vehiculo(garage, "PQO697", true);
  comprobar_vehiculo(garage,"PQO697");
  printf("\n");
