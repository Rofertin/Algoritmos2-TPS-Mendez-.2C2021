...
void abb_concadaelemento_inorden(nodo_abb_t* nodo, bool (*funcion)(void *, void *), void *aux, size_t* i){
    if(nodo){
      abb_concadaelemento_inorden(nodo->izquierda, funcion, aux, i);
      if (!funcion(nodo->elemento,aux)){
        *i += 1;
        return;
      }
      *i += 1;
      abb_concadaelemento_inorden(nodo->derecha, funcion, aux, i);
    }
}
void abb_concadaelemento_preorden(nodo_abb_t* nodo, bool (*funcion)(void *, void *), void *aux, size_t* i){
    if(nodo){
      if (!funcion(nodo->elemento,aux)){
        *i += 1;
        return;
      ...
      *i += 1;
      abb_concadaelemento_preorden(nodo->izquierda, funcion, aux, i);
      abb_concadaelemento_preorden(nodo->derecha, funcion, aux, i);
    }

}
void abb_concadaelemento_postorden(nodo_abb_t* nodo, bool (*funcion)(void *, void *), void *aux, size_t* i){
    if(nodo){
      abb_concadaelemento_postorden(nodo->izquierda, funcion, aux, i);
      abb_concadaelemento_postorden(nodo->derecha, funcion, aux, i);
      if(!funcion(nodo->elemento,aux)){
        *i += 1;
        return;
      }
...
size_t abb_con_cada_elemento(abb_t *arbol, abb_recorrido recorrido, bool (*funcion)(void *, void *), void *aux){
  ...
  size_t i = 0;
  if(recorrido == 0){ 
    abb_concadaelemento_inorden(arbol->nodo_raiz, funcion, aux, &i);
    printf("\ncantidad = %li",i);
  }
  if(recorrido == 1){ 
    abb_concadaelemento_preorden(arbol->nodo_raiz, funcion, aux, &i);
    printf("\ncantidad = %li",i);
  }
  if(recorrido == 2){ 
    abb_concadaelemento_postorden(arbol->nodo_raiz, funcion, aux, &i);
    printf("\ncantidad = %li",i);
  }
  return i;
}
